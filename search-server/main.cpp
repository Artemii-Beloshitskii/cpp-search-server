...
public:
    ...
    explicit SearchServer(const StringContainer& stop_words)
        ...
        for (const string& word : stop_words_) {
            if (!none_of(word.begin(), word.end(), [](char c) {
                return c >= '\0' && c < ' '; })) {
                throw invalid_argument("Wrong symbols in stop_words container"s);
            }
        }
    }
    ...
    void AddDocument(int document_id, const string& document, DocumentStatus status,
        ...
        if (document_id < 0) {
            throw invalid_argument("Trying to add a negative id"s);
        }
        if (documents_.count(document_id)) {
            throw invalid_argument("Trying to add id which is already exists"s);
        }
